/**
 * @fileOverview Anime tickle reaction.
 */
import axios from 'axios';

export default {
  name: "tickle",
  category: "anime",
  description: "Tickle someone with an anime GIF.",
  usage: "tickle <tag/reply>",
  cooldown: 5,
  permissions: 1,
  execute: async (ctx) => {
    const botName = await ctx.bot.managers.settings.get('core', 'name') || ctx.bot.config.name;
    let target = ctx.msg.message?.extendedTextMessage?.contextInfo?.participant || 
                 ctx.msg.message?.extendedTextMessage?.contextInfo?.mentionedJid?.[0];

    if (!target) return ctx.reply("┌──⌈ ⚠️ ERROR ⌋\n┃ Tag/Reply to tickle someone!\n└────────────────");

    const sources = ['https://api.waifu.pics/sfw/tickle', 'https://nekos.best/api/v2/tickle'];

    for (const url of sources) {
      try {
        const res = await axios.get(url);
        const gif = res.data.url || res.data.results[0].url;

        return await ctx.sock.sendMessage(ctx.jid, { 
          video: { url: gif },
          gifPlayback: true,
          caption: `┌──⌈ 😂 TICKLE ⌋\n┃ @${ctx.sender.split('@')[0]} tickles @${target.split('@')[0]}!\n└─ 🌌 ${botName}`,
          mentions: [target, ctx.sender]
        }, { quoted: ctx.msg });
      } catch (e) { continue; }
    }
    ctx.reply("┌──⌈ ⚠️ ERROR ⌋\n┃ Failed to load GIF.\n└────────────────");
  }
};