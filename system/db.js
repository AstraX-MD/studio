...
class RamStore {
  ...
  async getUser (jid) {
    return this._users.get(jid) || { xp: 0, coins: 0 }
  }

  async setUser (jid, key, value) {
    const user = this._users.get(jid) || { xp: 0, coins: 0 }
    user[key] = value
...
class MongoStore {
  ...
  async getUser (jid) {
    return (await this._users.findOne({ jid })) || { xp: 0, coins: 0 }
  }

  async setUser (jid, key, value) {
    await this._users.updateOne({ jid }, { $set: { [key]: value } }, { upsert: true })
    return true
  }
...
export const db = {
  ...
  async getUser (jid) { return _store.getUser(jid) },
  async setUser (jid, key, value) { return _store.setUser(jid, key, value) }
}